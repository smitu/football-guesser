...
export default function App() {
  ...
  if (screen === "menu") {
    const modes = [
      { key: "minute", icon: "⏱️", title: "ZGADNIJ MINUTĘ", desc: "Typuj minuty bramek na osi czasu", onClick: startGame },
      { key: "score", icon: "🏟️", title: "ZGADNIJ WYNIK", desc: "Podaj dokładny wynik meczu", onClick: () => setGameMode("score") },
      { key: "scorer", icon: "🎯", title: "ZGADNIJ STRZELCA", desc: "Kto strzelił legendarną bramkę?", onClick: () => setGameMode("scorer") },
      { key: "transfer", icon: "🔄", title: "ZGADNIJ ZAWODNIKA", desc: "Rozpoznaj piłkarza po transferach", onClick: () => setGameMode("transfer") },
      { key: "learn", icon: "📚", title: "TRYB NAUKI", desc: "Przepisy, historia, pojęcia bukmacherskie", onClick: () => setGameMode("learn") },
      { key: "stadium", icon: "🏟️", title: "ZGADNIJ STADION", desc: "Odkrywaj wskazówki i zgadnij stadion", onClick: () => setGameMode("stadium") },
    ];
      ...
      <div style={S.app}>
        <div style={{ ...S.card, ...S.center, marginTop: 24 }}>
          ...
          <h1 style={{
            ...
            background: "linear-gradient(135deg, #4ade80 0%, #22c55e 40%, #fbbf24 100%)",
          ...
          <p style={{
            color: "#4b5264", fontSize: 11, letterSpacing: 3,
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            {modes.map((mode, i) => {
              const accent = modeAccents[mode.key];
              return (
                <button key={mode.key} onClick={mode.onClick} style={{
                  ...modeButtons[mode.key],
                  textAlign: "left",
                  display: "flex", flexDirection: "column", gap: 8,
                  padding: "16px 14px",
                  animation: "fadeInUp 0.4s cubic-bezier(0.22, 1, 0.36, 1) both",
                  animationDelay: `${i * 0.05}s`,
                  gridColumn: mode.key === "learn" || mode.key === "stadium" ? "span 1" : undefined,
                }}>
                  <div style={{
                  ...
                  <div>
                    <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: 0.3, color: accent }}>{mode.title}</div>
                    <div style={{ fontSize: 10, fontWeight: 400, color: "#4b5264", marginTop: 3, lineHeight: 1.4, letterSpacing: 0, textTransform: "none" }}>{mode.desc}</div>
                  </div>
                </button>
              );
            })}
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
            <button style={{ ...S.ghostBtn, flex: 1, fontSize: 12, padding: "11px 16px" }} onClick={() => setScreen("leaderboard")}>🏆 Ranking</button>
            <button style={{ ...S.ghostBtn, flex: 1, fontSize: 12, padding: "11px 16px" }} onClick={() => setScreen("howto")}>❓ Jak grać</button>
          </div>
        </div>
        ...
        <div style={{ ...S.card, marginTop: 32 }}>
          ...
          {steps.map((s, i) => (
            <div key={i} style={{
              ...
              <div>
                <div style={{ fontWeight: 700, fontSize: 15, fontFamily: "'Bricolage Grotesque', sans-serif", marginBottom: 4 }}>{s.title}</div>
                <div style={{ color: "#4b5264", fontSize: 13, lineHeight: 1.5 }}>{s.desc}</div>
              </div>
            </div>
          ))}
            ...
            <div key={i} style={leaderboardRow(i)}>
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                <span style={{
                  ...
                  color: i === 0 ? "#fbbf24" : i < 3 ? "#4ade80" : "#4b5264",
    ...
    return (
      <div style={S.app}>
        <div style={{ ...S.card, ...S.center, marginTop: 32 }}>
          ...
          <div style={{ fontSize: 12, color: "#4b5264", marginTop: 4, fontFamily: "'Bricolage Grotesque', sans-serif", letterSpacing: 2 }}>TRYB: ZGADNIJ MINUTĘ</div>
          ...
          <div style={{
            ...
            <div style={{ fontSize: 52, fontWeight: 900, color: "#4ade80", fontFamily: "'Bricolage Grotesque', sans-serif", animation: "countUp 0.5s ease-out" }}>{totalScore}</div>
            <div style={{ color: "#4b5264", fontSize: 12, marginTop: 4, letterSpacing: 2, fontFamily: "'Bricolage Grotesque', sans-serif" }}>PUNKTÓW</div>
          </div>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 40, marginBottom: 28 }}>
            <div>
              ...
              <div style={{ color: "#4b5264", fontSize: 10, letterSpacing: 2, fontFamily: "'Bricolage Grotesque', sans-serif" }}>MECZY</div>
            </div>
            <div>
              <div style={{ fontSize: 24, fontWeight: 800, color: "#fbbf24", fontFamily: "'Bricolage Grotesque', sans-serif" }}>{perfectHits}</div>
              <div style={{ color: "#4b5264", fontSize: 10, letterSpacing: 2, fontFamily: "'Bricolage Grotesque', sans-serif" }}>IDEALNE</div>
            </div>
          </div>
          ...
          {matchResults.map((mr, i) => (
            <div key={i} style={summaryMatchRow}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <div style={{ fontWeight: 800, color: getScoreColor(mr.score), fontFamily: "'Bricolage Grotesque', sans-serif" }}>{mr.score} pkt</div>
              </div>
              <div style={{ fontSize: 11, color: "#4b5264", marginTop: 4 }}>
    ...
    return (
      <div style={S.app}>
        ...
        <div style={topBar}>
          ...
          <div style={{ fontSize: 13, color: "#4b5264", fontFamily: "'Bricolage Grotesque', sans-serif", fontWeight: 600 }}>Mecz {matchNum}/{matches.length}</div>
        ...
        <div style={{ ...S.card, position: "relative" }}>
          ...
          <div style={{
            ...
            <div style={{ fontSize: 15, fontWeight: 600, lineHeight: 1.5 }}>
              ...
              <span style={{ color: "#4ade80", fontWeight: 700 }}>{currentGoal.scorer}</span>
              <span style={{ color: "#4b5264", fontWeight: 400 }}> ({currentGoal.team === "home" ? currentMatch.home : currentMatch.away})</span>?
            </div>
          </div>
          ...
          {!showResult && (
            <div style={{ marginBottom: 24 }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#4b5264", marginBottom: 10, fontFamily: "'Bricolage Grotesque', sans-serif", fontWeight: 600 }}>
                <span>1'</span>
                <span>45'</span>
              ...
              >
                ...
                <div style={{
                  ...
                  background: "linear-gradient(135deg, #4ade80, #00c853)",
                  boxShadow: "0 0 20px rgba(0, 230, 118, 0.4), 0 2px 8px rgba(0, 0, 0, 0.3)",
                  display: "flex", alignItems: "center", justifyContent: "center",
                  transition: isDragging ? "none" : "left 0.1s",
                }}>
                  <span style={{ fontSize: 10, fontWeight: 800, color: "#08090c", fontFamily: "'Bricolage Grotesque', sans-serif" }}>{guessedMinute}'</span>
                </div>
              </div>
            </div>
          )}
          ...
          {showResult && lastScore && (
            <div style={resultBox(getScoreColor(lastScore.score))}>
              <div style={{ fontSize: 30, fontWeight: 900, color: getScoreColor(lastScore.score), fontFamily: "'Bricolage Grotesque', sans-serif" }}>
                {getScoreLabel(lastScore.score)}
              </div>
              {lastScore.timeOut ? (
                <div style={{ fontSize: 14, color: "#4b5264", marginTop: 10 }}>
                  Czas minął! Bramka padła w <b style={{ color: "#edf0f7" }}>{lastScore.actual}'</b>
                </div>
              ) : (
                <div style={{ fontSize: 14, color: "#4b5264", marginTop: 10 }}>
                  Twój typ: <b style={{ color: "#edf0f7" }}>{lastScore.guess}'</b> · Faktycznie: <b style={{ color: "#edf0f7" }}>{lastScore.actual}'</b> · Różnica: <b style={{ color: getScoreColor(lastScore.score) }}>{Math.abs(lastScore.guess - lastScore.actual)} min</b>
                </div>
              )}
              <div style={{ fontSize: 36, fontWeight: 900, color: getScoreColor(lastScore.score), marginTop: 10, fontFamily: "'Bricolage Grotesque', sans-serif" }}>+{lastScore.score}</div>
              ...
              <div style={{ position: "relative", height: 28, background: "rgba(255, 255, 255, 0.04)", borderRadius: 6, marginTop: 18, overflow: "visible" }}>
                {!lastScore.timeOut && (
                  <div style={{
                    ...
                    <span style={{ fontSize: 7, fontWeight: 800, color: "#fff", fontFamily: "'Bricolage Grotesque', sans-serif" }}>TY</span>
                  </div>
                )}
                <div style={{
                  ...
                  <span style={{ fontSize: 7, fontWeight: 800, color: "#08090c", fontFamily: "'Bricolage Grotesque', sans-serif" }}>{lastScore.actual}'</span>
                </div>
              </div>
            </div>
          )}
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 6, marginTop: 18 }}>
            {matches.map((_, i) => (
              <div key={i} style={progressDot(
                i < currentMatchIdx,
                i === currentMatchIdx,
                showResult ? getScoreColor(lastScore?.score || 0) : "#fbbf24"
              )} />
            ))}
          </div>
        </div>
