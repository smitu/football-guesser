    ...
    return (
      <div style={S.app}>
        <div style={{ ...S.card, ...S.center, marginTop: 32 }}>
          ...
          <div style={{ fontSize: 12, color: "#8892a4", marginTop: 4, fontFamily: "'Bricolage Grotesque', sans-serif", letterSpacing: 2 }}>TRYB: ZGADNIJ WYNIK</div>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 40, marginBottom: 28 }}>
            ...
            <div>
              <div style={{ fontSize: 24, fontWeight: 800, color: "#76ff03", fontFamily: "'Bricolage Grotesque', sans-serif" }}>{correctWinners}</div>
              <div style={{ color: "#4b5264", fontSize: 10, letterSpacing: 2, fontFamily: "'Bricolage Grotesque', sans-serif" }}>ZWYCIĘZCA</div>
            </div>
          </div>
          ...
          {matchResults.map((mr, i) => (
            <div key={i} style={summaryMatchRow}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <div style={{ fontWeight: 800, color: getScoreColor(mr.points), fontFamily: "'Bricolage Grotesque', sans-serif" }}>{mr.points} pkt</div>
              </div>
    ...
    return (
      <div style={S.app}>
        ...
        <div style={topBar}>
          ...
          <div style={{ fontSize: 13, color: "#8892a4", fontFamily: "'Bricolage Grotesque', sans-serif", fontWeight: 600 }}>Mecz {matchNum}/{matches.length}</div>
        ...
        <div style={{ ...S.card, position: "relative" }}>
          ...
          <div style={{ ...S.center, marginBottom: 28 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 20 }}>
              ...
              <span style={{ fontSize: 18, fontWeight: 800, color: "#2a3444", padding: "4px 14px", fontFamily: "'Bricolage Grotesque', sans-serif" }}>vs</span>
          ...
          {!showResult && (
            <div style={{ marginBottom: 24 }}>
              <div style={{ ...S.center, fontSize: 14, color: "#8892a4", marginBottom: 18 }}>Jaki był wynik tego meczu?</div>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 16 }} onKeyDown={handleKeyDown}>
                <div style={{ ...S.center }}>
                  <div style={{ fontSize: 11, color: "#8892a4", marginBottom: 10, fontWeight: 700, fontFamily: "'Bricolage Grotesque', sans-serif", letterSpacing: 1 }}>{currentMatch.home.split(" ").pop()}</div>
                  <input
                    ref={homeInputRef}
                    type="text"
                    inputMode="numeric"
                ...
                <span style={{ fontSize: 36, fontWeight: 900, color: "#2a3444", marginTop: 24, fontFamily: "'Bricolage Grotesque', sans-serif" }}>:</span>
                <div style={{ ...S.center }}>
                  <div style={{ fontSize: 11, color: "#8892a4", marginBottom: 10, fontWeight: 700, fontFamily: "'Bricolage Grotesque', sans-serif", letterSpacing: 1 }}>{currentMatch.away.split(" ").pop()}</div>
                  <input
                    ref={awayInputRef}
                    type="text"
                    inputMode="numeric"
          ...
          {showResult && lastResult && (
            <div style={resultBox(getScoreColor(lastResult.points))}>
              <div style={{ fontSize: 30, fontWeight: 900, color: getScoreColor(lastResult.points), fontFamily: "'Bricolage Grotesque', sans-serif" }}>{lastResult.label}</div>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 24, marginTop: 18 }}>
                {!lastResult.timeOut && (
                  <div>
                    <div style={{ fontSize: 10, color: "#4b5264", marginBottom: 6, letterSpacing: 2, fontFamily: "'Bricolage Grotesque', sans-serif" }}>TWÓJ TYP</div>
                    <div style={{ fontSize: 30, fontWeight: 900, color: "#8892a4", fontFamily: "'Bricolage Grotesque', sans-serif" }}>{lastResult.guessHome}:{lastResult.guessAway}</div>
                  </div>
                )}
                {!lastResult.timeOut && <div style={{ fontSize: 20, color: "#2a3444" }}>→</div>}
                <div>
                  <div style={{ fontSize: 10, color: "#4b5264", marginBottom: 6, letterSpacing: 2, fontFamily: "'Bricolage Grotesque', sans-serif" }}>{lastResult.timeOut ? "WYNIK" : "FAKTYCZNIE"}</div>
                  <div style={{ fontSize: 30, fontWeight: 900, color: getScoreColor(lastResult.points), fontFamily: "'Bricolage Grotesque', sans-serif" }}>{lastResult.actualHome}:{lastResult.actualAway}</div>
                </div>
              </div>
              {lastResult.timeOut && <div style={{ fontSize: 13, color: "#8892a4", marginTop: 8 }}>Czas minął!</div>}
              <div style={{ fontSize: 36, fontWeight: 900, color: getScoreColor(lastResult.points), marginTop: 14, fontFamily: "'Bricolage Grotesque', sans-serif" }}>+{lastResult.points}</div>
              <div style={{ fontSize: 11, color: "#4b5264", marginTop: 8, fontFamily: "'Sora', sans-serif" }}>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 6, marginTop: 18 }}>
            {matches.map((_, i) => (
              <div key={i} style={progressDot(
                i < currentMatchIdx,
                i === currentMatchIdx,
                showResult ? getScoreColor(lastResult?.points || 0) : "#fbbf24"
              )} />
            ))}
          </div>
        </div>
